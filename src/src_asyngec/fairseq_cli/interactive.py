...
Batch = namedtuple("Batch", "ids src_tokens src_lengths constraints src_nt src_nt_lengths src_outcoming_arc_mask src_incoming_arc_mask src_dpd_matrix src_probs_matrix tag_tokens")
...
def make_batches(lines, args, task, max_positions, encode_fn):
    ...
    if args.conll_file:
        ...
        if args.tag_file:
            # 句法概率矩阵(Soft GCN)
            src_tag_dataset = []
            src_tag_paths = args.tag_file
            for src_tag_path in src_tag_paths:
                if indexed_dataset.dataset_exists(src_tag_path, impl="mmap"):
                    src_tag_dataset.append(data_utils.load_indexed_dataset(
                        src_tag_path, None, "mmap"
                    ))
                else:
                    print(src_tag_path)
                    raise FileNotFoundError
    ...
    itr = task.get_batch_iterator(
        dataset=task.build_dataset_for_inference(
            tokens, lengths, constraints=constraints_tensor, src_tag_dataset = src_tag_dataset, src_nt=src_nt, src_nt_sizes=src_nt_sizes, src_conll_dataset=src_conll_dataset, src_dpd_dataset=src_dpd_dataset, src_probs_dataset=src_probs_dataset, syntax_type=args.syntax_type
    ...
    for batch in itr:
        ...
        if "tag_tokens" in batch["net_input"].keys():
            tag_tokens = batch['net_input']['tag_tokens']
