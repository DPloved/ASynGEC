...
class GCNSyntaxGuidedTransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, h, src_outcoming_arc_x=None, src_incoming_arc_x=None, encoder_padding_mask=None, attn_mask: Optional[Tensor] = None, src_probs_matrix: Optional[Tensor] = None,pos: Optional[Tensor] = None):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_padding_mask (ByteTensor): binary ByteTensor of shape
        ...
        length = src_outcoming_arc_x.size(1)
        if h.size(1) != length:
            
            h = h.mean(dim=1, keepdim=True)
            h = h.repeat(1,length,1)
...
用GCN对句法知识进行编码表示
"""
class wordGuidedTransformerEncoderLayer(nn.Module):
    """Encoder layer block.
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, h, src_outcoming_arc_x=None, src_incoming_arc_x=None, encoder_padding_mask=None, attn_mask: Optional[Tensor] = None, src_probs_matrix: Optional[Tensor] = None):
       ...
       # h = h.unsqueeze(1)  # B x 1 x T x D
        ...
        # h= h.unsqueeze(1)
        # h = h.repeat(1,seq_len,1,1)
        # B,T,T,D = src_outcoming_arc_x.size()
        # combined_out = torch.zeros(B, seq_len+T, seq_len+T, D).to(dtype=src_outcoming_arc_x.dtype)
        # combined_out[:, :seq_len, :seq_len, :] = h
        # combined_out[:, seq_len:seq_len+T, seq_len:seq_len+T, :]= src_outcoming_arc_x
        # combined_in = torch.zeros(B, seq_len+T, seq_len+T, D).to(dtype=src_outcoming_arc_x.dtype)
        # combined_in[:, :seq_len, :seq_len, :] = h
        # combined_in[:, seq_len:seq_len+T, seq_len:seq_len+T, :]= src_incoming_arc_x

        # combined_in=combined_in.to(device=src_incoming_arc_x.device)
        # combined_out=combined_in.to(device=src_outcoming_arc_x.device)
 #       h = h.repeat(1, seq_len, 1, 1)  # B x T x T x D, h[i,:,j]均为第i个句子的第j个token的嵌入表示
        ...
        if self.syntax_type == "dep":  # 依存句法需要考虑标签，同时考虑父亲节点和孩子节点
            # 对应论文式(1)
           # U_out = torch.cat((h, src_outcoming_arc_x),-1 )  # B x T x T x 2D
            U_out = self.W_out(src_incoming_arc_x)  # B x T x T x D
            # U_out = self.W_out(h)  # B x T x T x D
            U_out = self.activation_fn(src_incoming_arc_x)
            U_out = self.activation_dropout_module(src_incoming_arc_x)  # B x T x T x D

            # 对应论文式(2)
         #   U_in = torch.cat((h, src_incoming_arc_x), -1)  # B x T x T x 2D
            U_in = self.W_in(src_incoming_arc_x)  # B x T x T x D
            # U_in = self.W_in(h)  # B x T x T x D
            U_in = self.activation_fn(src_incoming_arc_x)
            U_in = self.activation_dropout_module(src_incoming_arc_x)  # B x T x T x D
           # src_probs_matrix = src_probs_matrix.unsqueeze(-1)
