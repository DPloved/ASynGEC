...
class SyntaxEnhancedTransformerEncoder(FairseqEncoder):
    ...
    def dual_aggregation(self, o1, o2, beta=0.5):
        if self.args.cross_syntax_fuse:
            ...
            #final_encoder_out = syntax_encoder_out
            return EncoderOut(
                encoder_out=final_encoder_out,  # T x B x C
                encoder_padding_mask=o1.encoder_padding_mask,  # B x T
                encoder_embedding=o1.encoder_embedding,  # B x T x C
                ...
                for i in range(len(self.syntax_encoder)):
                    if self.args.syntax_type[i] == "dep":
                        ...
                        syntax_encoder_out_all.append(self.syntax_encoder[i](
                            src_tokens, h=h,tag_tokens=tag_tokens, src_outcoming_arc_mask=src_outcoming_arc_mask[i], src_incoming_arc_mask=src_incoming_arc_mask[i], src_lengths=src_lengths, src_probs_matrix=src_probs_matrix[i], return_all_hiddens=return_all_hiddens
                        ))
                ...
                if self.args.syntax_type[0] == "dep":
                  #  assert src_incoming_arc_mask[0].shape[1] == src_tokens.shape[1]  # 确保tokens和掩码矩阵是同一个batch
                    syntax_encoder_out_all.append(self.syntax_encoder(
                            src_tokens, h=h,tag_tokens=tag_tokens,src_outcoming_arc_mask=src_outcoming_arc_mask[0], src_incoming_arc_mask=src_incoming_arc_mask[0], src_lengths=src_lengths, src_probs_matrix=src_probs_matrix[0], return_all_hiddens=return_all_hiddens
                        ))
                else:
                    if source_tokens_nt is not None:  # 把non-terminal节点加到句尾的scheme
                        i = 0
...
class SyntaxEnhancedSyntaxGuidedTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, syntax_type="dep"):
        ...
        self.padding_idx = embed_tokens.padding_idx
        self.max_source_positions = args.max_source_positions
        self.word_embed = nn.Embedding(num_embeddings=67, embedding_dim=1024)
        self.embed_tokens = embed_tokens
        self.embed_scale = 1.0 if args.no_scale_embedding else math.sqrt(embed_dim)  # if full mask then embed_scale must be 1
        ...
        self.word_embed_positions = None
#(
 #           PositionalEmbedding(
  #              args.max_source_positions,
   #             embed_dim,
      ...
      #      )
       #     )
        ...
        if self.encoder_layerdrop > 0.0:
            ...
            self.layers = nn.ModuleList([])
        self.layers.extend(
            [self.build_encoder_layer(args) for i in range(3)]
        )
        self.num_layers = len(self.layers)
    ...
    def forward_pos_embedding(
        ...
        # embed tokens and positions
    #    if token_embedding is None:
        token_embedding = self.word_embed(src_tokens)
        x = embed = self.embed_scale * token_embedding 
        if self.word_embed_positions is not None:
            x = embed + self.word_embed_positions(src_tokens)
        if self.layernorm_embedding is not None:
            x = self.layernorm_embedding(x)
        x = self.dropout_module(x)
        if self.quant_noise is not None:
    ...
    def forward(
        ...
        src_lengths,
        h,
        tag_tokens=None,  # Sentence Encoder 的输出, T x B x C
        src_outcoming_arc_mask = None,  # 描述源端句法树，出弧（指向孩子节点）掩码矩阵
        src_incoming_arc_mask = None,  # 描述源端句法树，入弧（指向孩子节点）掩码矩阵
        ...
        pos,_=self.forward_pos_embedding(tag_tokens)
        ...
        encoder_states = [] if return_all_hiddens else None
        #if self.training:
         #   src_probs_matrix = self.SRC_dropout(src_probs_matrix, 0.2)
        # encoder layers
        ...
        #h = pos+h
        h=h.transpose(1,0)
        pos=pos.transpose(1,0)  # Layer norm是否对Batch First生效？
        lengh,_,_=redual.size()
        h = h.mean(dim=0, keepdim=True)
        h = h.repeat(lengh,1,1)
        pos = pos.mean(dim=0, keepdim=True)
        pos = pos.repeat(lengh,1,1)
        fusion=h+redual+pos
        
        redual_pool =self.pool(fusion)
        redual_sigmod = torch.sigmoid(redual_pool)
       # sig = torch.mul(fusion,redual_sigmod)
       # h = sig *redual
        h = self.lin(self.tanh(self.norm( torch.mul(fusion,redual_sigmod)+redual)))
